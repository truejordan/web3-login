...
const ActivityItem = ({ item }: { item: any }) => {
  ...
  const iotUrl = `${IotaConfigType.blockExplorerUrl.devnet.replace("?network=devnet", "")}/txblock/${item.id}?network=devnet`;
  // const suiUrl = `${SuiConfigType.blockExplorerUrl.devnet}/tx/${item.id}`; //use for sui
  return (
    <TouchableOpacity
      onPress={() =>
        WebBrowser.openBrowserAsync(iotUrl, {
...
const Activity = () => {
  const { accountActivity } = useW3IotaAuth();
  // removes duplicate transactions when sending
  const deduplicatedActivity = accountActivity.reduce(
    (acc, current) => {
