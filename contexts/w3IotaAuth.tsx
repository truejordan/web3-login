...
const chainConfig = {
  ...
  chainId: IotaConfigType.chainId.devnet,
  // Avoid using public rpcTarget in production.
  // Use services like Infura, QuickNode, etc.
  rpcTarget: IotaConfigType.rpcTarget.devnet,
  displayName: IotaConfigType.displayName.devnet,
  blockExplorerUrl: IotaConfigType.blockExplorerUrl.devnet,
  ticker: IotaConfigType.ticker,
  tickerName: IotaConfigType.ticker,
  logo: IotaConfigType.logo,
};
...
const W3IotaAuthContext = createContext<W3IotaAuthContextType | undefined>(
  undefined
);

export const W3IotaAuthProvider = ({
  children,
}: {
  ...
  // Create an instance of the IOTA local key pair manager for signing transactions
  ...
  const getAddress = useCallback(async () => {
    ...
    setAddress(address);
    uiConsole(`IOTA account: ${address}`);
    console.log(`IOTA account: ${address}`);
    return address;
  }, [getKeyPair, clearKeyPair, uiConsole]);

  // Get the IOTA RPC client
  const iotaRPC = useCallback(() => {
    if (!provider) {
  ...
  const getBalance = useCallback(
    async (isPolling: boolean = false) => {
      ...
      try {
        ...
        const iotaBalance = balance(balanceResponse as CoinBalance);
        setMybalance(iotaBalance);
        // only log balance when called not from interval polling
        if (!isPolling) {
          uiConsole(`Iota Balance: ${iotaBalance}`);
    ...
    [provider, address, iotaRPC, uiConsole]
  );

  // rquest IOTA fromfuacet devnet/testnet
  ...
  const sendTransaction = async (
    ...
    try {
      ...
      const [coin] = tx.splitCoins(tx.gas, [
        tx.pure.u64(amount * Number(NANOS_PER_IOTA)),
      ]);
      ...
      const result = await iotaRPC()?.signAndExecuteTransaction({
        signer: kp,
        transaction: tx,
      });
  ...
  const signMessage = async (message?: string) => {
    ...
    try {
      ...
      const result: SignMessageResult = {
        ...
        publicKey: kp.getPublicKey().toIotaPublicKey(),
        address: kp.toIotaAddress(),
        timestamp,
      };
  ...
  return (
    <W3IotaAuthContext.Provider value={value}>
      {children}
    </W3IotaAuthContext.Provider>
  );
};

export const useW3IotaAuth = () => {
  const context = useContext(W3IotaAuthContext);
  if (!context) {
    throw new Error("useW3IotaAuth must be used within a W3IotaAuthProvider");
  }
  return context;
};
